...
export const createRecipe = (req, res) => {
	...
	pool
		.query(
			...
			[title, ingredients, instructions, home_picture, home_picture_alt, picture_one, picture_one_alt, picture_two, picture_two_alt]
		)
		.then((data) => res.json('hihihi'))
		.catch((err) => console.log(err));
};

export const deleteSingleRecipe = (req, res) => {
	const id = req.params.id;
	pool
		.query('DELETE FROM recipes WHERE id=$1', [id])
		.then(res.send('erfolgreich gelöscht'))
